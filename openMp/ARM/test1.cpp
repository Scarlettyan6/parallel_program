...
void gaussian_parallel_col(int n) {
    int i, j, k;
    float tmp;
...
int main() {
    printf("Matrix Size\tSerial (ms)\tRow-Parallel (ms)\tCol-Parallel (ms)\n");

    for (int n = STEP; n <= 2024; n += STEP) {
        init_matrix(n);
        ...
        gaussian_parallel_col(n); // uses A
